...
grammar = """
    ...
    added grammar for switch statement
...
def parse_case_clause(tokens):
    ...
    assert tokens[0]["tag"] == ":", f"Expected ':' after case expression at position {tokens[0]['position']}"
    tokens = tokens[1:]
    case_body, tokens = parse_statement_list(tokens)
    return {"tag": "case", "value": case_value, "body": case_body}, tokens
...
def parse_default_clause(tokens):
    ...
    tokens = tokens[1:]
    assert tokens[0]["tag"] == ":", f"Expected ':' after default at position {tokens[0]['position']}"
    tokens = tokens[1:]
    default_body, tokens = parse_statement_list(tokens)
    return {"tag": "default", "body": default_body}, tokens
...
def parse_switch_statement(tokens):
    ...
    tokens = tokens[1:]
    assert tokens[0]["tag"] == "(", f"Expected '(' after switch at position {tokens[0]['position']}"
    tokens = tokens[1:]
    switch_expr, tokens = parse_expression(tokens)
    assert tokens[0]["tag"] == ")", f"Expected ')' after switch expression at position {tokens[0]['position']}"
    tokens = tokens[1:]
    assert tokens[0]["tag"] == "{", f"Expected '{{' to start switch block at position {tokens[0]['position']}"
    tokens = tokens[1:]
    ...
    while tokens[0]["tag"] != "}":
        if tokens[0]["tag"] == "case":
            ...
            assert default_case is None, f"Multiple default clauses found at position {tokens[0]['position']}"
            ...
            if tokens[0]["tag"] == ";":
                 tokens = tokens[1:]
                 continue
            raise Exception(f"Expected 'case', 'default', or '}}' inside switch block, got '{tokens[0]['tag']}' at position {tokens[0]['position']}")


    assert tokens[0]["tag"] == "}", f"Expected '}}' to end switch block at position {tokens[0]['position']}"
    tokens = tokens[1:]
...
def test_parse_switch_statement():
    ...
    """
    ast1, _ = parse_switch_statement(tokenize(code1))

    # Basic structural checks - more detailed checks might be needed
    assert ast1["tag"] == "switch"
    assert ast1["expression"]["tag"] == "identifier"
    assert len(ast1["cases"]) == 2
    assert ast1["cases"][0]["tag"] == "case"
    assert ast1["cases"][0]["value"]["tag"] == "number"
    assert ast1["cases"][0]["value"]["value"] == 1
    assert ast1["cases"][0]["body"]["tag"] == "statement_list"
    assert len(ast1["cases"][0]["body"]["statements"]) == 2 # print and break
    assert ast1["cases"][1]["tag"] == "case"
    assert ast1["cases"][1]["value"]["value"] == 2
    assert ast1["default"]["tag"] == "default"
    assert ast1["default"]["body"]["tag"] == "statement_list"

    code2 = "switch(y) {}" # Empty switch
    ast2, _ = parse_switch_statement(tokenize(code2))
    assert ast2["tag"] == "switch"
    assert len(ast2["cases"]) == 0
    assert ast2["default"] is None

    code3 = "switch(z) { default: {x=1;} }" # Only default
    ast3, _ = parse_switch_statement(tokenize(code3))
    assert ast3["tag"] == "switch"
    assert len(ast3["cases"]) == 0
    assert ast3["default"] is not None
